...
/**
 * `quiet` marks the sheet's own doing — a peer ghost offered, the pinned minute preselected —
 * which applies without making the sheet `dirty`: only the author's hand makes a Post worth a
 * leave prompt.
 */
...
/** What every panel of one Report is handed. Mirrors mobile's `SectionProps`. */
export interface SectionProps {
  report: SheetReport;
  body: SheetBody | null;
  /** Advance this Report's reducer. */
  dispatch: (action: SheetAction, opts?: DispatchOpts) => void;
  /** Change what rides beside the reducer (photos, the bundle choice). */
