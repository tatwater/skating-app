...
import { postSheet, saveSheetAsDraft, saveSheetEdit } from '../../lib/sheetActions';
import {
  addEarlierVisit,
  addLake,
  type PostSheet,
...
function SheetBody({ post, onDone }: { post: PostSheet; onDone: () => void }) {
  ...
  const onSaveDraft = async () => {
    ...
    setBusy('draft');
    try {
      const saved = await saveSheetAsDraft(post, Date.now());
      setSheet(saved);
      refresh();
      setSheet(null);
      onDone();
      ...
      setMessage("Couldn't save this draft. Please try again.");
    } finally {
      setBusy(null);
    }
  };
          ...
          if (picking?.kind === 'add') {
            ...
            updateSheet((p) =>
              updateReport(p, reportId, (r) => ({
                ...r,
                bodyName: body.name,
                sheet: { ...r.sheet, waterBodyId: body.waterBodyId },
                coord: undefined,
              })),
            );
          }
