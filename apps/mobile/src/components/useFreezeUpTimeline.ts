 ...
 *
 * ## Deliberately a copy of web's, and only this file
 *
 * The fetching, caching and folding all live in `@skating/core` — this is the React lifecycle around
 * them, and core carries no React dependency to host it. What is duplicated is a hundred lines of
 * effect wiring whose *decisions* are all one package over, so the two can differ in how they
 * schedule without being able to differ about what the archive says.
 */
...
export function useFreezeUpTimeline({
  ...
  useEffect(() => {
    ...
    // left `loading` stuck true forever, showing "Loading the freeze-up timeline…" in place of
  ...
  // worse, handed back a fresh `stops` array, which made every consumer's effect deps change on
  // renders that had nothing to do with the archive (the frame prefetch aborted and restarted, the
  // auto-select effect re-ran). `statsVersion` is bumped as each manifest lands, so the progressive
  // upgrade still happens — it just no longer happens for free.
