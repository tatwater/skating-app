...
describe('resolveAuthRoute', () => {
  ...
  it('sends signed-out users to the auth flow', () => {
    expect(resolveAuthRoute({ isLoaded: true, isSignedIn: false, profile: undefined })).toBe('auth')
  })
  ...
  it('routes a signed-in user with no profile row to onboarding', () => {
    expect(resolveAuthRoute({ isLoaded: true, isSignedIn: true, profile: null })).toBe('onboarding')
  })
