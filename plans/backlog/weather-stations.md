# Next-gen — Weather stations: learning how a bay's weather really differs from its water body's
...
> *"Can we get more accurate weather from Open-Meteo in a bay vs the whole water body? … If we find a good
...
> differs from the parent water body, and use the difference trend to make better predictions in future
> seasons based on the parent's historical data?"* — 2026-09-16

Three parts, three different answers.

**1. Is a bay's Open-Meteo weather finer than its water body's? Forecast yes, marginally; history mostly
no.** Since A06h (D152) every bay is already sampled at its **own** point — Champlain's ten bays sit
in ten distinct 5.6 × 4 km browse cells, none of which is the water body's. Open-Meteo's US forecast is
...
**3. Can model-at-bay minus model-at-body teach us a per-bay correction? No — and this is the part
...
| **Tempest (WeatherFlow)** and **Weather Underground PWS** | personal weather stations, often literally on a dock | dozens on the big water bodies; coverage is where people live, which is where they skate | Tempest: public station API, no key for public stations; WU: key, generous free tier |
| **CoCoRaHS** | citizen daily precip/snow depth | dense in VT/NH/ME | daily CSV export |
| **Water body buoys** | UVM's Lake Champlain buoys, NDBC | summer-only; pulled each fall | irrelevant for ice, recorded so nobody looks twice |
