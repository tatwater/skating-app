...
no.** Since N6h (D152) every bay is already sampled at its **own** point — Champlain's ten bays sit
...
opened. N9's Workstream G closes it: one Tier A fetch per live bay per day, all season, ~128
calls/day.
...
| --- | --- | --- | --- |
| **NWS / ASOS + AWOS** | the official airport stations; hourly wind speed/dir, temp, precip | BTV (Burlington, on Champlain's shore), MPV, RUT, LEB, CON, ASH, PSM, PWM, AUG, BGR, GFL, SLK, MSS, ALB… ~40 in five states | `api.weather.gov/stations/{id}/observations` — free, no key, the same API N6c's NWS alerts already read |
