...
🟡 **In progress** 2026-09-21 · PRs #71 #72 #73 #75 #77 #78 #79 · [plan](./phases/A10-reporting-flow.md) · D186–D200 D203 D204 D205 D206
...
- 🟢 **Photos that assign themselves, place-mode, the photo ↔ hazard hand-offs, GPX import** — A10-7 (2026-09-23): by capture time within a Report's window, else by location on a lake (placed by it only on the water, D42 amendment); an amber `?` with a menu on the photo for the rest (the lakes, the put-in, the lot, *this is a hazard*, leave it out); pool photos sent to a launch filed as access photos after the Post; the camera-roll reel over the skate's window on the phone (`expo-media-library`); a hazard opened from a photo, and a pin offered the Post's photos within 150 m; a real *Add your track* — a GPX file, parsed on the client and ingested as an activity. Most of §8.1 and the GPX half of §4.2
- ❓ **Strava import** — the connection is write-only by decision (`strava.ts`: no use for reading anyone's activities); pulling an activity would add `activity:read` to the consent and re-connect every linked account. The GPX export covers it today (Strava exports one per activity); a founder call whether the scope is worth it
...
| **End-to-end tests** — Playwright (web), Maestro (the preview APK), and the RN render-test harness (`@testing-library/react-native` installed, unused; zero `.test.tsx` on mobile); the on-ice flow "via Maestro" 09a promised. **The sheet is rewritten on both surfaces now** (A10-3, A10-5), so *post a two-lake Post* is the first flow worth a Playwright pass | ⚪ | flows stabilizing after the alpha | [`backlog/e2e-tests.md`](./backlog/e2e-tests.md) |
