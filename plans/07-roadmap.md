...
- **PR 2 ✅ built:** `activity_detected` from the recorder's un-prompted skates (hourly sweep, 3 h
  ...
  zone** (`profiles.timezone` from the device; the hour stays 20:00 — true-sunset dropped, D170).
- **PR 3 ✅ built (D171):** the transports — Expo Push (server posts to `exp.host`; dead tokens
...
- **Push credentials + store credentials.** ~~Remote push delivery — no token registration, no
  APNs/FCM credentials, no server sender.~~ **Built in N8 PR 3 (D171):** token registration, an Expo
