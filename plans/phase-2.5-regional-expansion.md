...
- **Read-cap at scale (D49).** A far bigger corpus stresses `listInViewport` harder at wide zoom — the
  in-query `minVisibleZoom` filter is exactly the fix, but **validate wide-zoom read counts against the
  full multi-state corpus** (not just VT) so the filter still shrinks reads rather than growing them.
