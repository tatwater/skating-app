...
| `CLERK_WEBHOOK_SIGNING_SECRET` (Convex env) | ⬜ dev — endpoint not yet registered | verifies `POST /clerk-webhook` (`user.updated` → email/avatar mirrors). Route answers 500 until set, on purpose. Per Clerk instance; §11b of the credentials doc. |
