# @skating/lake-depth — lake-depth ETL (A06a)
...
by hand. See [`plans/phases/A06a-body-depth.md`](../../plans/phases/A06a-body-depth.md) for the why.
...
> requests, so there is no reason to skip it. The A07a runbook was missing this line until 2026-08-26.
>
...
bodies *with their polygons* first; doing it in Convex costs ~8k small indexed lookups against the A01 cell
...
are permanently out of scope. See [`plans/phases/A06b-bathymetry-layer.md`](../../plans/phases/A06b-bathymetry-layer.md).
...
2. **Read the *Intellectual Rights* statement and copy it.** This is the
   [open question this phase has never resolved](../../plans/phases/A06a-body-depth.md#open-questions),
...
3. Download **four of the package's entities** — not just the data. Under 2.5 MB in total, and three
   ...
   | **`data_dictionary_depth`** (8.4 KiB) | The authoritative column names **and units**, which is half of [this phase's open question](../../plans/phases/A06a-body-depth.md#open-questions). Reading it beats inferring units from a header. |
...
Write the numbers into [`plans/phases/A06a-body-depth.md`](../../plans/phases/A06a-body-depth.md) whichever way
they come out — including "7 m was fine", which is a result and not a non-event.

> **Why this is a runbook step and not a note in a plan doc.** Phase 07-2 built the `photo_orphans` metric
> *and* an index expressly to decide whether a cron was worth writing, and nobody pointed at either for
> months (see the A03 phase doc). An evidence gate nobody points at is not a gate. This one sits in the
...
- **No contours.** Bathymetric isobaths are [A06b](../../plans/phases/A06b-bathymetry-layer.md) — a different
...
- **No state-agency bulk depths.** Rung 2 (`state_agency`) exists in the enum and has no loader yet: NH
  GRANIT / VT ANR / MassGIS / NYSDEC are being fetched in A06b anyway for their contours, and parsing them
...
- **No OSM depth tags.** `osm_tag` is carried by the **water** ETL (`scripts/etl`, `--depths` →
  ...
  A06a review after the register spent a phase claiming it was already folded in. Inland coverage is
