...
decibels to gray. The tempting choice is a per-scene stretch — take each pass's own minimum and
...
Normalizing each frame to its own extremes makes every frame look about the same and hides exactly the
...
apart can be compared by eye because the same gray means the same backscatter.
...
**It does not decide anything.** The gray is a picture; the measurement is `sar-zonal.py`'s per-body
    ...
    for y in range(0, height, ROWS_PER_WINDOW):
        ...
        if usable.any():
            ...
            if noise_band is not None:
                ...
                # a picture: a pixel needs *a* gray, and there is no honest gray for negative power.
