...
#
# Write one `importRuns` row for a push, via the run-log CLI (A06c F2).
#
...
mirror_main() {
  ...
  case "$mode" in
    push)
      ...
      # File the receipt (A06c F2). The push is the step that makes an archive durable, and "when did
